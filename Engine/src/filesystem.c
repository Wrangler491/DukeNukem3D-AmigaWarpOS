...
#if defined(__APPLE__) || defined(__linux__) || defined(WARPUP)
...
int32_t uncompress(uint8_t  *lzwinbuf, int32_t compleng, uint8_t  *lzwoutbuf)
{
	...
	strtot = (int32_t ) BUILDSWAP_INTEL16(shortptr[1]);
	if (strtot == 0)
	{
		copybuf((void *)((lzwinbuf)+4),(void *)((lzwoutbuf)),((compleng-4)+3)>>2);
		return((int32_t ) BUILDSWAP_INTEL16(shortptr[0])); /* uncompleng */
	}
...
{
	//kread(fil, buffer, count*dasizeof);
	...
	kread16(fil,&leng); kread(fil,lzwbuf5,(int32_t )leng);
	k = 0;
...
{
	//kdfread(buffer, dasizeof, count, fil);
...
void dfwrite(void *buffer, size_t dasizeof, size_t count, FILE *fil)
{
	...
	if (k > LZWSIZE-dasizeof)
	{
		...
		fwrite(&byteswapped,2,1,fil); fwrite(lzwbuf5,(int32_t )leng,1,fil);
	}
