              ...
              int32_t *daposz, short *daang, short *dacursectnum)
{
    ...
    {
		kread(fil,tbuf,10+11*2+2*4);
		tmp16=(*tbuf++)+(*tbuf++<<8);
		sect->wallptr=tmp16;
		tmp16=(*tbuf++)+(*tbuf++<<8);
        sect->wallnum=tmp16;
		tmp32=(*tbuf++)+(*tbuf++<<8)+(*tbuf++<<16)+(*tbuf++<<24);
        sect->ceilingz=tmp32;
		tmp32=(*tbuf++)+(*tbuf++<<8)+(*tbuf++<<16)+(*tbuf++<<24);
        sect->floorz=tmp32;
		tmp16=(*tbuf++)+(*tbuf++<<8);
        sect->ceilingstat=tmp16;
		tmp16=(*tbuf++)+(*tbuf++<<8);
        sect->floorstat=tmp16;
		tmp16=(*tbuf++)+(*tbuf++<<8);
        sect->ceilingpicnum=tmp16;
		tmp16=(*tbuf++)+(*tbuf++<<8);
        sect->ceilingheinum=tmp16;
        sect->ceilingshade=*tbuf++;
        sect->ceilingpal=*tbuf++;
        sect->ceilingxpanning=*tbuf++;
        sect->ceilingypanning=*tbuf++;
		tmp16=(*tbuf++)+(*tbuf++<<8);
        sect->floorpicnum=tmp16;
		tmp16=(*tbuf++)+(*tbuf++<<8);
        sect->floorheinum=tmp16;
        sect->floorshade=*tbuf++;
        sect->floorpal=*tbuf++;
        sect->floorxpanning=*tbuf++;
        sect->floorypanning=*tbuf++;
        sect->visibility=*tbuf++;
        sect->filler=*tbuf++;
		tmp16=(*tbuf++)+(*tbuf++<<8);
        sect->lotag=tmp16;
		tmp16=(*tbuf++)+(*tbuf++<<8);
...
static void loadpalette(void)
{
    ...
    for (k = 0; k < (65536 / 4); k++) {
		...
		tmp32=(*tptr++)+(*tptr<<8)+(*tptr++<<16)+(*tptr++<<24);
		*tptr2++=tmp32;
	}
