...

{
    ...
    do
    {
        ...
        if ((fil = TCkopen4load(artfilename,0)) != -1)
        {
			...
			buf = malloc((localtileend-localtilestart+1)<<2);
            kread(fil,buf,(localtileend-localtilestart+1)<<1);
			tptr = buf;
            for (i = localtilestart; i <= localtileend; i++) {
                //read16(fil,&tiles[i].dim.width);
				tiles[i].dim.width = (*tptr++)+(*tptr++<<8);
			}
			//free(buf);
            
			//buf = malloc((localtileend-localtilestart+1)<<1);
            kread(fil,buf,(localtileend-localtilestart+1)<<1);
			tptr = buf;
            for (i = localtilestart; i <= localtileend; i++) {
                //kread16(fil,&tiles[i].dim.height);
				tiles[i].dim.height = (*tptr++)+(*tptr++<<8);
			}
			//free(buf);
            
			//buf = malloc((localtileend-localtilestart+1)<<2);
            kread(fil,buf,(localtileend-localtilestart+1)<<2);
			tptr = buf;
            for (i = localtilestart; i <= localtileend; i++) {
                //kread32(fil,&tiles[i].animFlags);
				tiles[i].animFlags = (*tptr++)+(*tptr++<<8)+(*tptr++<<16)+(*tptr++<<24);
			}
			free(buf);
