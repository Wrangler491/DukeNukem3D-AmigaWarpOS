...
static int sdl_mouse_motion_filter(SDL_Event const *event)
{
    ...
    if (event->type == SDL_JOYBALLMOTION)
    {
		...
		//printf("joyball motion: mrx=%d, mry=%d, mx=%d, my=%d\n",mouse_relative_x, mouse_relative_y, event->jball.xrel, event->jball.yrel);
    }
    else
    {
...
static int sdl_key_filter(const SDL_Event *event)
{
    ...
    if ( (event->key.keysym.sym == SDLK_m) &&
         ...
         (event->key.keysym.mod & KMOD_ALT) )	//was KMOD_CTRL
    {
...
void getvalidvesamodes(void)
{
	...
	sdl_flags = BFullScreen ? SDL_FULLSCREEN : 0; //	reset fullscreen from config
	sdl_flags |= SDL_HWPALETTE;
