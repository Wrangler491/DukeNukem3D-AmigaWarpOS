...
void precachenecessarysounds(void)
{
    ...
    for(i=0;i<NUM_SOUNDS;i++) {
        if(Sound[i].ptr == 0)
        {
		    ...
		    if(SoundToggle != 0) {
			    fp = TCkopen4load(sounds[i],0);
    			if(fp == -1) break;
    			l = kfilelength( fp );
    			soundsiz[i] = l;

			    if( (ud.level_number == 0 && ud.volume_number == 0 && (i == 189 || i == 232 || i == 99 || i == 233 || i == 17 ) ) ||
   			     ( l < 12288 ) )
   			 {	
			        Sound[i].lock = 2;
   			     allocache(&Sound[i].ptr,l,&Sound[i].lock);
     			   if(Sound[i].ptr != NULL) {
        			    kread( fp, Sound[i].ptr , l);
					}
    			}
    			kclose( fp );
			}
