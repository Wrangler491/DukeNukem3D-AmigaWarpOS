...
#ifdef _WIN32
  #include <windows.h>
#elif defined(__APPLE__)  || defined(WARPUP)
  #include "SDL.h"
#endif
...
int main(int argc,char  **argv)
{
	...
	printf("*** Chocolate DukeNukem3D Amiga WarpOS v%d.%d ***\n\n", CHOCOLATE_DUKE_REV_X, CHOCOLATE_DUKE_REV_DOT_Y);
...
/*
     ...
     kread32(recfilep,(int32 *)&dummy /*ud.auto_run*/); // not needed and would affect autorun status in duke3d.cfg when quitting duke from a demo
     ...
     for(i=0;i<ud.multimode;i++)
	 {
        kread8(recfilep,(int32 *)&ps[i].aim_mode);
...
// CTW END - MODIFICATION
    ...
    fwrite(&ver,sizeof(uint8_t),1,frecfilep);
		// FIX_00062: Better support and identification for GRP and CON files for 1.3/1.3d/1.4/1.5
	val32 = BUILDSWAP_INTEL32(groupefil_crc32);
	fwrite((int32_t *)&val32,sizeof(int32_t),1,frecfilep);
    fwrite((uint8_t *)&ud.volume_number,sizeof(uint8_t),1,frecfilep);
    fwrite((uint8_t *)&ud.level_number,sizeof(uint8_t),1,frecfilep);
    fwrite((uint8_t *)&ud.player_skill,sizeof(uint8_t),1,frecfilep);
    fwrite((uint8_t *)&ud.m_coop,sizeof(uint8_t),1,frecfilep);
	...
	{    
		...
		fwrite(&val32,sizeof(uint8_t),1,frecfilep);
		// FIX_00080: Out Of Synch in demos. Tries recovering OOS in old demos v27/28/29/116/117/118. New: v30/v119.
		val32 = BUILDSWAP_INTEL32(ud.wchoice[i]);
		fwrite(&val32,sizeof(ud.wchoice[0]),1,frecfilep);
	}
...
void closedemowrite(void)
{
		...
		inptr++;
            ...
            fwrite(&val32,sizeof(int32_t),1,frecfilep);
