...
static int MV_cubic16(const signed short *src, int position, int rate)
{
	int temp, hpos = position >> 16;
   ...
   uint32_t rate, const char *start, uint32_t length )
{
	...
	for (i = 0; i < length; i++) {
		signed short s = (src[(position>>16)]>>8)&0xff | (src[position>>16]<<8)&0xff00;
		double left, right;
